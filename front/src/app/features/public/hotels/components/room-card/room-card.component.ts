...
@Component({
  ...
  template: `
    <article class="room" [class.selected]="isRoomSelected()">
      <div class="head">
        <img class="image" [src]="room().images[0]?.src || '/images/hotels/hotel-room-v2.png'" [alt]="room().images[0]?.alt || room().name" loading="lazy" />
        <div>
  ...
  styles: `
    ...
    .head > div:last-child { align-self: center; }
