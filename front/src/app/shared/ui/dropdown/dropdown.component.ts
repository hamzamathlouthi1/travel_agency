...
export class DropdownComponent {
  ...
  onHostClick(event: MouseEvent): void {
    // Opens the panel when the trigger is clicked. Guarded on `!open()` so
    // this never fights with a consumer that already manages opening itself
    // (e.g. destination-field's own trigger click handler) — it only fills
    // in the open behavior for consumers that were relying on us for it.
    if (this.open()) return;
    const trigger = this.host.nativeElement.querySelector('[dropdownTrigger]') as HTMLElement | null;
